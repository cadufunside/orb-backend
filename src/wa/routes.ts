...
export function waRoutes(gw: WaGateway, bus: EventEmitter, publish: (ch:string, p:any)=>void) {
  ...
  r.post('/sessions/:id/start', async (req, res) => {
    const { id } = req.params;
    try { await gw.start(id); res.json({ ok: true }); }
  ...
  r.post('/sessions/:id/messages/text', async (req, res) => {
    ...
    try { const s = gw.get(id); const jid = formatJid(to); await s.sendText(jid, text); res.json({ ok: true }); }
  ...
  bus.on('wa:inbound', ({ sessionId, message }: any) => {
    ...
    publish(`session:${sessionId}:contact:${jid}`, { type: 'message', direction: 'in', message });
  });
